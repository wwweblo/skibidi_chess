...
function getLinkedPositions(parentId: number): Set<number> {
  const rows = db
    .prepare("SELECT child_id FROM position_links WHERE parent_id = ?")
...
function getPositionId(fen: string): number | null {
  const result = db
    .prepare("SELECT id FROM Openings WHERE fen = ?")
    .get(fen) as { id: number } | undefined;
  return result ? result.id : null;
}

function insertPositionLink(parentId: number, childId: number, move: string): void {
  db.prepare(
    "INSERT OR IGNORE INTO position_links (parent_id, child_id, move) VALUES (?, ?, ?)"
...
// ✅ Явно указываем, что это ES-модуль
export default checkAndGenerateLinks;
